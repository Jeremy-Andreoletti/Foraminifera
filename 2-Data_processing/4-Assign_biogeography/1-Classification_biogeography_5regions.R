...
Biogeography_data <- Env_data %>% select(`Original Sample ID`, Averaged_Latitude, Averaged_Longitude, Province_Spalding_simplified_02) %>% 
  dplyr::rename("Biogeography" = "Province_Spalding_simplified_02", "latitude" = "Averaged_Latitude", "longitude" = "Averaged_Longitude") 

# Create a color palette
pal <- colorFactor(palette = c("#1B9E77", "#D95F02", "#7570B3", "#E7298A", "#E6AB02"), domain = Biogeography_data$Biogeography)
...
m <- leaflet(Biogeography_data) %>%
  ...
  addLegend("bottomright", pal = pal, values = ~Biogeography,
            title = "Biogeography")
m
mapshot(m, file = "~/Nextcloud/Recherche/2_Plankton/Forams/3-Data_processed/Biogeography/Map_biogeography_classification_5regions.pdf")
...
write_xlsx(
  ...
  path ="~/Nextcloud/Recherche/2_Plankton/Forams/3-Data_processed/Biogeography/Occurence_Table_5regions.xlsx")
