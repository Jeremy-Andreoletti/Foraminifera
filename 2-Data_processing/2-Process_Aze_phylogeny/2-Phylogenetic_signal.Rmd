...
sampling_heterogeneity <- read.csv("../../3-Data_processed/Triton_occurrences/sampling_heterogeneity.csv", row.names=1)
...
match_fossil_to_last_tip <- as.character(sapply(rownames(dat.discrete), function(sp_name) first(grep("T", Foram_ancestry_table[grep(sp_name,last_tip), "Lineage.code"][[1]], value=T))))
dat.discrete <- dat.discrete[!is.na(match_fossil_to_last_tip),] ; match_fossil_to_last_tip <- match_fossil_to_last_tip[!is.na(match_fossil_to_last_tip)]   # remove unmatched fossils
row.names(dat.discrete) <- match_fossil_to_last_tip
head(dat.discrete)
```

```{r fig.width=12, fig.asp = .6}
Foram_phylo <- read.tree("../../3-Data_processed/Morphospecies_phylogenies/Forams_bifurcating.tree")
trait.plot(tree=keep.tip(phy=Foram_phylo, tip=rownames(dat.discrete)), dat=dat.discrete,
                        cols=list(reason=c("pink", "palevioletred", "red", "red4"), db.source=c("lightblue", "skyblue", "blue", "darkblue")))
trait.plot(tree=force.ultrametric(keep.tip(phy=Foram_phylo, tip=rownames(dat.discrete))), dat=dat.discrete,
                        cols=list(reason=c("pink", "palevioletred", "red", "red4"), db.source=c("lightblue", "skyblue", "blue", "darkblue")))
```
...
dat.continuous <- sampling_heterogeneity[sampling_heterogeneity$MacroMicro=="Macroperforate",c("pal.lat", "pal.long", "pal.long.var", "age", "sample.depth")]
...
match_fossil_to_last_tip <- as.character(sapply(rownames(dat.continuous), function(sp_name) first(grep("T", Foram_ancestry_table[grep(sp_name,last_tip), "Lineage.code"][[1]], value=T))))
dat.continuous <- dat.continuous[!is.na(match_fossil_to_last_tip),] ; match_fossil_to_last_tip <- match_fossil_to_last_tip[!is.na(match_fossil_to_last_tip)]   # remove unmatched fossils
