...
sqsMyrBinsPlot <- subsample(triton_divDyn, bin="MyrBin", tax="species", coll="holeID", q=0.7, iter=100, ref="holeID", singleton="ref", type="sqs", duplicates=FALSE, excludeDominant=TRUE, largestColl =TRUE, output="dist", na.rm=TRUE)
```
...
tsplot(stages, shading="short", boxes=c("short", "sys"), boxes.col=c("col", "systemCol"), xlim=c(66,0), ylim=c(0,120), labels.args=list(cex=0.7), ylab="Range-through diversity")
title("Comparison between estimates of foraminifera richness trends")
lines(sampMyrBin$MyrBin, ddMyrBins$divRT, col="darkgreen", lwd=2)

# loop through all trial results
shades(sampMyrBin$MyrBin, sqsMyrBinsPlot$divRT, col="palegreen4")
legend("topleft", bg="white", legend=c("raw data", "experts' LTT", "SQS trials"), col=c("darkgreen", "orchid4", "palegreen4"), lwd=3, inset=c(0.01, 0.01, 0.01))

#the mean of the trial results
meanSQS <- rowMeans(sqsMyrBinsPlot$divRT, na.rm = T)
lines(sampMyrBin$MyrBin, meanSQS, col="palegreen4", lwd=2)
