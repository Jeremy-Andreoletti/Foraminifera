...
p <- ggplot(triton[triton$Speciation<66,], aes(x=(Speciation-age_runif)/(Speciation-Extinction), fill=as.factor(ceiling((Speciation+Extinction)/2/10)*10))) + 
  ...
  scale_x_continuous(limits=c(-1,2), name = "Expert-curated species lifetime", breaks = c(0,1), labels = c("Speciation", "Ext. / Now")) + 
...
ggsave("Images/Fig1C_Lifetime_occurrence_distribution.pdf", p + ggtitle("Raw Triton database"), width = 4, height = 3)
ggsave("Images/Fig1C_Lifetime_occurrence_distribution_extinct.pdf", q + ggtitle("Raw Triton database, extinct species"), width = 4, height = 3)
...
ggsave("Images/Fig1C_Lifetime_occurrence_distribution_subsampled_extinct.pdf", q + ggtitle("Subsampled Triton database, extinct"), width = 4, height = 3)
...
# Keep valid coords
samples <- triton.ss.withinRanges %>%
  filter(
    is.finite(latitude),  is.finite(longitude),
    is.finite(pal.lat),   is.finite(pal.long)
  )

# Coastlines only
coast <- rnaturalearth::ne_coastline(scale = "medium", returnclass = "sf")

# Common theme with compact margins
theme_map_compact <- theme_minimal(base_size = 12) +
  theme(
    panel.spacing = unit(0, "lines"),
    panel.grid = element_blank(),
    axis.title = element_text(size = 12),
    plot.title = element_text(size = 13, hjust = 0.5, margin = margin(b = 2))
  )

# Present-day coordinates + coastlines
p_now <- ggplot() +
  geom_bin2d(data = samples, aes(x = longitude, y = latitude), bins = 45) +
  geom_sf(data = coast, inherit.aes = FALSE, alpha = 0.5,
          color = "grey20", linewidth = 0.25, fill = NA) +
  coord_sf(xlim = c(-180, 180), ylim = c(-90, 90), expand = FALSE) +
  scale_fill_gradient(low = "grey90", high = "steelblue4",
                      trans = "log10", name = "Count") +
  labs(title = "Spatial distribution of samples (present-day coordinates)",
       x = "Longitude (°)", y = "Latitude (°)") +
  theme_map_compact

# Paleocoordinates (add faint modern coastline outline for reference)
p_paleo <- ggplot() +
  geom_bin2d(data = samples, aes(x = pal.long, y = pal.lat), bins = 45) +
  geom_sf(data = coast, inherit.aes = FALSE,
          color = "grey30", linewidth = 0.25,
          fill = NA, alpha = 0.25) +
  coord_sf(xlim = c(-180, 180), ylim = c(-90, 90), expand = FALSE, clip = "off") +
  scale_fill_gradient(low = "grey90", high = "steelblue4",
                      trans = "log10", name = "Count") +
  labs(title = "Spatial distribution of samples (paleocoordinates)",
       x = "Paleolongitude (°)", y = "Paleolatitude (°)") +
  theme_map_compact

gridExtra::grid.arrange(p_now, p_paleo, nrow = 1)

# Save
ggsave("Images/Annex_Spatial_distribution_samples.pdf",
       gridExtra::arrangeGrob(p_now, p_paleo, nrow = 1),
       width = 12, height = 3)
```
...
median_abundant_region <- function(pal.lat_i){
...
sampling_heterogeneity$pal.lat = sapply(sampling_heterogeneity$species, 
                                        function(species_i)median_abundant_region(triton.ss.withinRanges[triton.ss.withinRanges$species==species_i,"pal.lat"]))
...
geo_predictors_increasing_sampling <- lapply(names(sort(table(triton.ss.withinRanges$species))), function(SP){
    triton.ss.SP <- triton.ss.withinRanges[triton.ss.withinRanges$species == SP,]
    #triton.ss.SP <- triton.ss.SP[!duplicated(triton.ss.SP$holeID), ]
    X <- 1:nrow(triton.ss.SP)
    triton.ss.SP <- triton.ss.SP[order(triton.ss.SP$age_runif),]
    return (Reduce(rbind, lapply(X, function(x)c(pal_lat_median  = median_abundant_region(triton.ss.SP$pal.lat[1:x]),
                                      pal_lat_var     = var(abs(triton.ss.SP$pal.lat[1:x])),
                                      pal_long_median = median.as.circular(triton.ss.SP$pal.long[1:x])[[1]],
                                      pal_long_var    = var.as.circular(triton.ss.SP$pal.long[1:x])
                                      ))))
})
```
...
```{r fig.width=10, fig.asp=0.8}
par(mfrow=c(2,2))
for (geo_predictors_increasing_sampling_i in tail(geo_predictors_increasing_sampling)){
  plot(geo_predictors_increasing_sampling_i[,"pal_lat_median"], xlab = "Cumulative number of samples (chronological)", ylab = "Median latitude in abundant region", main = paste0("Median lat. convergence"), ylim = c(-90,90))
  plot(geo_predictors_increasing_sampling_i[,"pal_lat_var"], xlab = "Cumulative number of samples (chronological)", ylab = "Variance in absolute latitude", main = paste0("Lat. variance convergence"), ylim = c(0,1000))
  plot(geo_predictors_increasing_sampling_i[,"pal_long_median"], xlab = "Cumulative number of samples (chronological)", ylab = "Circular median longitude", main = paste0("Median long. convergence"), ylim = c(-180,180))
  plot(geo_predictors_increasing_sampling_i[,"pal_long_var"], xlab = "Cumulative number of samples (chronological)", ylab = "Circular variance in longitude", main = paste0("Long. variance convergence"), ylim = c(0,1))
}
```

```{r warning=FALSE, fig.width=12}
plot_overlaid_trajectories <- function(traj_list, var, ylab = var, ylim = NULL,
                                       main = NULL,
                                       line_col = adjustcolor("black", 0.15),
                                       med_col  = "firebrick",
                                       band_col = adjustcolor("firebrick", 0.18),
                                       center_to_final = FALSE,   # <— NEW
                                       zero_line = TRUE) {        # <— NEW
  # Keep only entries with the column present
  traj_list <- lapply(traj_list, function(x) as.data.frame(x, check.names = FALSE))
  has_var <- vapply(traj_list, function(x) var %in% names(x), logical(1))
  if (!all(has_var)) {
    warning("Dropping ", sum(!has_var), " species without column '", var, "'.")
    traj_list <- traj_list[has_var]
  }
  if (!length(traj_list)) stop("No species left with column '", var, "'")

  # Optional centering to the final value per species
  if (center_to_final) {
    traj_list <- lapply(traj_list, function(df) {
      y <- df[[var]]
      y0 <- if (all(is.na(y))) NA_real_ else tail(y[!is.na(y)], 1)
      if (is.finite(y0)) df[[var]] <- y - y0
      df
    })
    if (is.null(ylim)) ylab <- paste0("Δ ", ylab)
  }

  # Basic limits
  lens <- vapply(traj_list, nrow, integer(1))
  maxL <- max(lens)
  xlim <- c(1, maxL)
  if (is.null(ylim)) {
    ylim <- range(unlist(lapply(traj_list, function(x) x[[var]])), na.rm = TRUE)
  }

  plot(NA, xlim = xlim, ylim = ylim,
       xlab = "Cumulative number of samples (chronological)",
       ylab = ylab, main = main)
  grid(col = adjustcolor("grey70", 0.6))
  if (center_to_final && zero_line) abline(h = 0, lty = 3, col = adjustcolor("grey20", 0.8))

  # Draw each species
  for (i in seq_along(traj_list)) {
    y <- traj_list[[i]][[var]]
    lines(seq_along(y), y, col = line_col, lwd = 1)
  }

  # Pointwise median/IQR where ≥1 species contributes
  k_counts <- integer(maxL)
  qs <- matrix(NA_real_, nrow = 3, ncol = maxL,
               dimnames = list(c("lo","med","hi"), NULL))
  for (k in 1:maxL) {
    vals <- vapply(traj_list, function(x) if (nrow(x) >= k) x[[var]][k] else NA_real_, numeric(1))
    vals <- vals[!is.na(vals)]
    k_counts[k] <- length(vals)
    if (k_counts[k] > 0) {
      qs[, k] <- c(quantile(vals, 0.25), median(vals), quantile(vals, 0.75))
    }
  }
  usable <- which(k_counts > 0L)
  if (length(usable) >= 2L) {
    polygon(c(usable, rev(usable)),
            c(qs["lo", usable], rev(qs["hi", usable])),
            col = band_col, border = NA)
    lines(usable, qs["med", usable], col = med_col, lwd = 2)
  } else if (length(usable) == 1L) {
    points(usable, qs["med", usable], pch = 19, col = med_col)
  }
}
...
plot_overlaid_trajectories(geo_predictors_increasing_sampling,
                           var = "pal_lat_median",
                           ylab = "Median paleolatitude (°)",
                           ylim = c(-90, 90),
                           main = "Median latitude")

# lat_var_range <- range(unlist(lapply(geo_predictors_increasing_sampling, function(x) if(is.vector(x)){x["pal_lat_var"]}else{x[, "pal_lat_var"]})), na.rm = TRUE)
plot_overlaid_trajectories(geo_predictors_increasing_sampling,
                           var = "pal_lat_var",
                           ylab = "Variance of |latitude| (°²)",
                           ylim = c(0,1000),
                           main = "Latitude variance")

plot_overlaid_trajectories(geo_predictors_increasing_sampling,
                           var = "pal_long_median",
                           ylab = "Circular median longitude (°)",
                           ylim = c(-180, 180),
                           main = "Median longitude")

plot_overlaid_trajectories(geo_predictors_increasing_sampling,
                           var = "pal_long_var",
                           ylab = "Circular variance (longitude)",
                           ylim = c(0, 1),
                           main = "Longitude variance")
```
...
plot_overlaid_trajectories(geo_predictors_increasing_sampling,
  ...
  ylab = "Median paleolatitude",
  ylim = NULL,
  main = "Median latitude (centered to final)",
  center_to_final = TRUE)

plot_overlaid_trajectories(geo_predictors_increasing_sampling,
  var = "pal_lat_var",
  ylab = "Variance of |latitude|",
  ylim = NULL,
  main = "Latitude variance (centered to final)",
  center_to_final = TRUE)
...
plot_overlaid_trajectories(geo_predictors_increasing_sampling,
  ...
  ylab = "Circular median longitude",
  ylim = NULL,
  main = "Median longitude (centered to final)",
  center_to_final = TRUE)
...
plot_overlaid_trajectories(geo_predictors_increasing_sampling,
  ...
  ylab = "Circular variance (longitude)",
  ylim = NULL,
  main = "Longitude variance (centered to final)",
  center_to_final = TRUE)

invisible(dev.print(pdf, "Images/Annex_geo_predictors_increasing_sampling.pdf"))
```

```{r fig.width=15, fig.asp=0.2, message=FALSE}
## Deduplicate by holeID and add decade
holes <- triton.ss.withinRanges %>%
  distinct(holeID, .keep_all = TRUE) %>%
  mutate(decade = 10 * floor(year / 10)) %>%
  filter(!is.na(decade)) %>%
  mutate(decade = factor(decade, levels = sort(unique(decade))))

## Latitude by decade
p <- ggplot(holes, aes(x = decade, y = latitude, group = decade)) +
  geom_violin(trim = FALSE, bounds = c(-90,90), bw=10, scale = "count", na.rm = TRUE) +
  ggtitle("Latitudinal distribution of holes over time") +
  labs(x = "Decade", y = "Hole sampling latitude (°)") +
  lims(y=c(-90,90)) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

## Longitude by decade
q <- ggplot(holes, aes(x = decade, y = longitude, group = decade)) +
  geom_violin(trim = FALSE, bounds = c(-180,180), bw=20, scale = "count", na.rm = TRUE) +
  lims(y=c(-180,180)) +
  ggtitle("Longitudinal distribution of holes over time") +
  labs(x = "Decade", y = "Hole sampling longitude (°)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggsave("Images/Annex_Hole_latitudinal_distribution_per_decade.pdf", p, width = 15, height = 3)
ggsave("Images/Annex_Hole_longitudinal_distribution_per_decade.pdf", q, width = 15, height = 3)
ggsave("Images/Hole_latitudinal_distribution_per_decade.png", p, width = 15, height = 3)
ggsave("Images/Hole_longitudinal_distribution_per_decade.png", q, width = 15, height = 3)
p
q
```
...
p <- grid.arrange(
  ggplot(sampling_heterogeneity, aes(x=pal.lat, y=sampling_rate, fill=pal.lat)) +
    ...
    ggtitle("Predictor: paleolatitude") +
  ...
  ggplot(sampling_heterogeneity, aes(x=pal.long, y=sampling_rate, fill=pal.long)) +
    ...
    scale_y_continuous(name = "Fossil sampling rate (per Myr)") +
    ggtitle("Predictor: paleolongitude") +
    theme(legend.position = "none"),
  ggplot(sampling_heterogeneity[sampling_heterogeneity$rel.abun<18,], aes(x=rel.abun, y=sampling_rate, fill=rel.abun)) +
    ...
    scale_y_continuous(name = "Fossil sampling rate (per Myr)") +
    ggtitle("Predictor: relative abundance") +
    theme(legend.position = "none"),
  ...
  ggplot(sampling_heterogeneity, aes(x=sample.depth, y=sampling_rate, fill=sample.depth)) +
    ...
    scale_y_continuous(name = "Fossil sampling rate (per Myr)") +
    ggtitle("Predictor: sample depth") +
    theme(legend.position = "none"), 
  ggplot(sampling_heterogeneity, aes(x=pal.absLat.var, y=sampling_rate, fill=sample.depth)) +
    geom_point() +
    stat_smooth(formula=y~s(x), method = "gam", method.args=list(family=Gamma(link = "log"))) +
    scale_x_continuous(name = "Paleolatitudinal spread") +
    scale_y_continuous(name = "Fossil sampling rate (per Myr)") +
    ggtitle("Predictor: paleolat. spread") +
    theme(legend.position = "none"), 
  ggplot(sampling_heterogeneity, aes(x=pal.long.var, y=sampling_rate, fill=sample.depth)) +
    ...
    scale_y_continuous(name = "Fossil sampling rate (per Myr)") +
    ggtitle("Predictor: plaeolong. spread") +
    theme(legend.position = "none"), 
  nrow=2)
...
# Type of sample
sampling_heterogeneity$db.source = sapply(sampling_heterogeneity$species, 
                                        function(species_i) as.factor(get_mode(triton.ss.withinRanges[triton.ss.withinRanges$species==species_i,"db.source"])))
```
...
Simplify to avoid groups with few datapoints.

```{r, fig.asp=0.4, fig.width=12}
sampling_heterogeneity <- sampling_heterogeneity %>% 
  mutate(
    morphogroup_simplified = case_when(
      morphogroup %in% c(1, 5, 6)    ~ "S_flat_clavate_planispiral",
      morphogroup %in% 2:4           ~ "S_globular_spherical",
      morphogroup %in% 7:8           ~ "NS_globular",
      morphogroup %in% 9:11          ~ "NS_spinate_planispiral",
      morphogroup %in% 12:16         ~ "NS_rotaliform",
      morphogroup %in% 17:19         ~ "NS_muricate",
      morphogroup =="Microperforate" ~ "Microperforate",
      TRUE                           ~ "NA"
    ) %>% factor(levels = c("NA", "S_globular_spherical", "S_flat_clavate_planispiral", "NS_globular", "NS_muricate", "NS_rotaliform", "NS_spinate_planispiral", "Microperforate"))
  )

# sampling_heterogeneity$morphogroup_simplified2 <- sampling_heterogeneity$morphogroup_simplified
# sampling_heterogeneity$morphogroup_simplified2 <- gsub("^S.*", "Spinose", sampling_heterogeneity$morphogroup_simplified2)
# sampling_heterogeneity$morphogroup_simplified2 <- gsub("^NS.*", "NonSpinose", sampling_heterogeneity$morphogroup_simplified2)

sampling_heterogeneity <- sampling_heterogeneity %>% 
  mutate(
    ecogroup_simplified = case_when(
      ecogroup ==1 ~ "Symbiotic_SML",
      ecogroup ==2 ~ "Asymbiotic_SML",
      ecogroup ==3 ~ "Thermocline",
      ecogroup ==4 ~ "Sub_thermocline",
      TRUE         ~ "NA"
    ) %>% factor(levels = c("NA", "Asymbiotic_SML", "Symbiotic_SML", "Sub_thermocline", "Thermocline"))
  )

# sampling_heterogeneity <- sampling_heterogeneity %>%
#   mutate(
#     morphogroup_simplified = relevel(morphogroup_simplified, ref = "S_globular_spherical"),
#     ecogroup_simplified    = relevel(ecogroup_simplified,    ref = "Asymbiotic_SML")
#   )

p <- grid.arrange(
  ggplot(sampling_heterogeneity, aes(x=morphogroup_simplified, y=sampling_rate, fill=morphogroup_simplified)) +
    geom_boxplot(varwidth=T, outlier.shape=NA) + 
    geom_jitter(color="black", size=0.4, alpha=0.5, width = 0.2) +
    scale_x_discrete(name = "Morphogroups") +
    scale_y_log10(name = "Fossil sampling rate (per Myr)") +
    ggtitle("Heterogeneity in fossil sampling") +
    theme(legend.position = "none", axis.text.x = element_text(angle = 20, vjust = 0.95, hjust=1)),
  ggplot(sampling_heterogeneity, aes(x=ecogroup_simplified, y=sampling_rate, fill=ecogroup_simplified)) +
    geom_boxplot(varwidth=T, outlier.shape=NA) + 
    geom_jitter(color="black", size=0.4, alpha=0.5, width = 0.2) +
    scale_x_discrete(name = "Ecogroups") +
    scale_y_log10(name = "Fossil sampling rate (per Myr)") +
    ggtitle("Heterogeneity in fossil sampling") +
    theme(legend.position = "none", axis.text.x = element_text(angle = 20, vjust = 0.95, hjust=1)), nrow=1, widths=c(8,4))
ggsave("Images/Annex_Plot_simplified_functional_groups.pdf", p, width = 10, height = 3.5)
```

```{r}
summary(lm(sampling_rate~relevel(morphogroup_simplified, ref = "NA"), data=sampling_heterogeneity))
anova(lm(sampling_rate~morphogroup_simplified, data=sampling_heterogeneity))
```

```{r}
summary(lm(sampling_rate~relevel(ecogroup_simplified, ref = "NA"), data=sampling_heterogeneity))
anova(lm(sampling_rate~ecogroup_simplified, data=sampling_heterogeneity))
```
...
```{r}
write.csv(sampling_heterogeneity, "../../3-Data_processed/Triton_occurrences/sampling_heterogeneity.csv")
```
...
sampling_heterogeneity_GLM <- glm(sampling_rate ~ rel.abun + abs(pal.lat) + pal.long + age + sample.depth + pal.absLat.var + pal.long.var + reason + age.calc + morphogroup_simplified + ecogroup_simplified,
...
```{r, fig.asp=0.4, fig.width=12, warning=FALSE}
sampling_heterogeneity_GAM <- gam(sampling_rate ~ s(pal.lat, pal.long, bs = "sos", k=50) + s(rel.abun, k=3) + s(pal.absLat.var, k=3) + s(pal.long.var, k=3) + s(sample.depth, k=3)
                                                  + reason + age.calc + morphogroup_simplified + ecogroup_simplified, 
                                  data   = sampling_heterogeneity[sampling_heterogeneity$rel.abun<25,],
...
for (i in 1:5) plot(sampling_heterogeneity_GAM, pages=0, residuals=TRUE, select=i, se=2, ylim=c(-3,1.5), theta=0, phi=90, n2=100, ylab="")  ## show partial residuals
# vis.gam(sampling_heterogeneity_GAM, view=c("age", "sample.depth"), n.grid = 15, lwd=0.5, ylab="\n\nsample depth")
for (i in 1:5) plot(sampling_heterogeneity_GAM, pages=0, seWithMean=TRUE, select=i, se=2, shade=T, theta=180, n2=100, phi=90, ylab="")
# vis.gam(sampling_heterogeneity_GAM, view=c("sample.depth", "age"), n.grid = 15, lwd=0.5, ylab="\n\nage")
```

```{r, fig.asp=0.4, fig.width=12, warning=FALSE}
pdf("Images/Fig2A_Sampling_heterogeneity_GAM.pdf", width = 12, height = 3.5)
layout(matrix(c(1, 2, 3, 4, 5,
                6, 2, 3, 4, 5), 
              nrow = 2, byrow = TRUE))

par(mar = c(0.5, 0.5, 0.5, 0.5), mgp = c(2.2, 0.7, 0))
plot(sampling_heterogeneity_GAM, pages=0, residuals=TRUE, seWithMean=TRUE, select=1, theta=0, phi=90, ylab="", n2=200, lwd=0.3, hcolors = viridis::viridis(100))

par(mar = c(3.5, 3.3, 1, 0), mgp = c(2, 0.5, 0))
plot(sampling_heterogeneity_GAM, pages=0, residuals=TRUE, shade=T, select=2, se=2, cex.lab=1.8,
     ylim=c(-3,1.5), theta=0, phi=90, xlab="Relative abundance", ylab="Partial effect of the predictor")

par(mar = c(3.5, 2, 1, 0.5), mgp = c(2, 0.5, 0))
plot(sampling_heterogeneity_GAM, pages=0, residuals=TRUE, shade=T, select=3, se=2, cex.lab=1.8, 
     ylim=c(-3,1.5), yaxt = "n", theta=0, phi=90, xlab="Latitudinal spread", ylab="")

par(mar = c(3.5, 1.5, 1, 0.5), mgp = c(2, 0.5, 0))
plot(sampling_heterogeneity_GAM, pages=0, residuals=TRUE, shade=T, select=4, se=2, cex.lab=1.8, 
     ylim=c(-3,1.5), yaxt = "n", theta=0, phi=90, xlab="Longitudinal spread", ylab="")

par(mar = c(3.5, 1.5, 1, 1), mgp = c(2, 0.5, 0))
plot(sampling_heterogeneity_GAM, pages=0, residuals=TRUE, shade=T, select=5, se=2, cex.lab=1.8, 
     ylim=c(-3,1.5), yaxt = "n", theta=0, phi=90, xlab="Sample Depth", ylab="")

# par(mar = c(1, 0, 0.5, 0))
# vis.gam(sampling_heterogeneity_GAM, view=c("age", "sample.depth"), n.grid = 7,
#         ylab="\nDepth", xlab="Age", zlab="Partial effect", cex.lab=1.2)
# 
plot(sampling_heterogeneity_GAM, pages=0, residuals=TRUE, select=1, theta=180, phi=90, ylab="", n2=200, lwd=0.3, hcolors = viridis::viridis(100))
...
grid.arrange(plot_model(sampling_heterogeneity_GLM, type="pred", terms = c("pal.lat"))+geom_point(),
             ...
             plot_model(sampling_heterogeneity_GLM, type="pred", terms = c("pal.absLat.var", "age.calc"))+geom_point(),
             plot_model(sampling_heterogeneity_GLM, type="pred", terms = c("sample.depth", "reason"))+geom_point(),
             plot_model(sampling_heterogeneity_GLM, type="pred", terms = c("rel.abun", "ecogroup_simplified"))+geom_point(),
             plot_model(sampling_heterogeneity_GLM, type="pred", terms = c("pal.long.var", "morphogroup_simplified"))+geom_point(), nrow=2)
...
grid.arrange(plot_model(sampling_heterogeneity_GAM, type="pred", terms = c("pal.lat"))+geom_point(),
             ...
             plot_model(sampling_heterogeneity_GAM, type="pred", terms = c("pal.absLat.var", "age.calc"))+geom_point(),
             plot_model(sampling_heterogeneity_GAM, type="pred", terms = c("sample.depth", "reason"))+geom_point(),
             plot_model(sampling_heterogeneity_GAM, type="pred", terms = c("rel.abun", "ecogroup_simplified"))+geom_point(),
             plot_model(sampling_heterogeneity_GAM, type="pred", terms = c("pal.long.var", "morphogroup_simplified"))+geom_point(), nrow=2)
```
...
```{r}
performance::check_collinearity(sampling_heterogeneity_GLM)
```


```{r}
performance::check_collinearity(sampling_heterogeneity_GAM)
cat("\n\n\n")
performance::check_concurvity(sampling_heterogeneity_GAM)
```

```{r}
1/(1-concurvity(sampling_heterogeneity_GAM))
```
...
sampling_heterogeneity_GAM.deviancePart <- sampling_heterogeneity_GAM.deviancePart %>%
  mutate(label = case_when(
    ...
    grepl("sample\\.depth", predictor)   ~ "Sample depth",
...
ggsave("Images/Fig2C_Deviance_partitionning.pdf", p, width = 6, height = 6)
p
```
...
for (r in seq_len(nrow(smooth_tab))) {
  ...
  } else if (term_name == "s(sample.depth)") {
    mapped_factor  <- "\\makecell[l]{Sample depth}"
    level_str      <- ""
    search_pattern <- "sample.depth"
  } else {
...
# Alternative analysis without relative abundance

```{r, fig.asp=0.4, fig.width=12, warning=FALSE}
sampling_heterogeneity_GAM_rmAbundance <- gam(sampling_rate ~ s(pal.lat, pal.long, bs = "sos", k=50) + s(pal.absLat.var, k=3) + s(pal.long.var, k=3) + s(sample.depth, k=3)
                                                  + reason + age.calc + morphogroup_simplified + ecogroup_simplified, 
                                  data   = sampling_heterogeneity[sampling_heterogeneity$rel.abun<25,],
                                  family = Gamma(link = "log"), method = "REML", select=T)
summary(sampling_heterogeneity_GAM_rmAbundance)

par(mfrow = c(2,4), mar = c(4, 3, 2, 0) + 0.1, mgp = c(2.2, 0.7, 0))
for (i in 1:4) plot(sampling_heterogeneity_GAM_rmAbundance, pages=0, residuals=TRUE, select=i, se=2, ylim=c(-3,1.5), theta=0, phi=90, ylab="")  ## show partial residuals
for (i in 1:4) plot(sampling_heterogeneity_GAM_rmAbundance, pages=0, seWithMean=TRUE, select=i, se=2, shade=T, theta=180, phi=90, ylab="")

pdf("Images/Annex_Sampling_heterogeneity_GAM_rmAbundance.pdf", width = 12, height = 5)
par(mfrow = c(2,4), mar = c(4, 3, 2, 0) + 0.1, mgp = c(2.2, 0.7, 0))
for (i in 1:5) plot(sampling_heterogeneity_GAM_rmAbundance, pages=0, residuals=TRUE, select=i, se=2, ylim=c(-3,1.5), theta=0, phi=90, ylab="")  ## show partial residuals
for (i in 1:5) plot(sampling_heterogeneity_GAM_rmAbundance, pages=0, seWithMean=TRUE, select=i, se=2, shade=T, theta=180, phi=90, ylab="")
dev.off()
```

```{r, fig.asp=0.4, fig.width=12, warning=FALSE}
pdf("Images/Annex_Sampling_heterogeneity_GAM_rmAbundance.pdf", width = 10, height = 4)
layout(matrix(c(1, 2, 3, 4,
                5, 2, 3, 4), 
              nrow = 2, byrow = TRUE))

par(mar = c(0.5, 0.5, 0.5, 0.5), mgp = c(2.2, 0.7, 0))
plot(sampling_heterogeneity_GAM_rmAbundance, pages=0, residuals=TRUE, seWithMean=TRUE, select=1, theta=0, phi=90, ylab="", lwd=0.3)

par(mar = c(3.5, 3.2, 1, 0.5), mgp = c(2, 0.5, 0))
plot(sampling_heterogeneity_GAM_rmAbundance, pages=0, residuals=TRUE, shade=T, select=2, se=2, cex.lab=1.7, 
     ylim=c(-3,1.5), theta=0, phi=90, xlab="Latitudinal spread", ylab="Partial effect of the predictor")

par(mar = c(3.5, 1.5, 1, 0.5), mgp = c(2, 0.5, 0))
plot(sampling_heterogeneity_GAM_rmAbundance, pages=0, residuals=TRUE, shade=T, select=3, se=2, cex.lab=1.7, 
     ylim=c(-3,1.5), yaxt = "n", theta=0, phi=90, xlab="Longitudinal spread", ylab="")

par(mar = c(3.5, 1.5, 1, 1), mgp = c(2, 0.5, 0))
plot(sampling_heterogeneity_GAM_rmAbundance, pages=0, residuals=TRUE, shade=T, select=4, se=2, cex.lab=1.7, 
     ylim=c(-3,1.5), yaxt = "n", theta=0, phi=90, xlab="Sample Depth", ylab="")

plot(sampling_heterogeneity_GAM_rmAbundance, pages=0, residuals=TRUE, select=1, theta=180, phi=90, ylab="", lwd=0.3)
dev.off()
```

```{r fig.asp=0.3, fig.width=15, echo = FALSE, eval = FALSE}
par(mfrow=c(1,4))
gam.check(sampling_heterogeneity_GAM_rmAbundance, rep = 100)
```

Analysis of the Variance Inflation Factor (VIF)

```{r}
performance::check_collinearity(sampling_heterogeneity_GAM_rmAbundance)
cat("\n\n\n")
performance::check_concurvity(sampling_heterogeneity_GAM_rmAbundance)
```

```{r, fig.width=10}
sampling_heterogeneity_GAM_rmAbundance.hp <- gam.hp(sampling_heterogeneity_GAM_rmAbundance, type="dev")
sampling_heterogeneity_GAM_rmAbundance.hp

plot(sampling_heterogeneity_GAM_rmAbundance.hp)
```

```{r, fig.width=20, warning=F, message=F}
sampling_heterogeneity_GAM_rmAbundance.deviancePart <- sampling_heterogeneity_GAM_rmAbundance.hp$hierarchical.partitioning[,"Individual"]
sampling_heterogeneity_GAM_rmAbundance.deviancePart <- c(sampling_heterogeneity_GAM_rmAbundance.deviancePart, residuals = 1-sum(sampling_heterogeneity_GAM_rmAbundance.deviancePart))
sampling_heterogeneity_GAM_rmAbundance.deviancePart <- data.frame(deviance = sampling_heterogeneity_GAM_rmAbundance.deviancePart, predictor=names(sampling_heterogeneity_GAM_rmAbundance.deviancePart))

sampling_heterogeneity_GAM_rmAbundance.deviancePart <- sampling_heterogeneity_GAM_rmAbundance.deviancePart %>%
  mutate(label = case_when(
    grepl("pal\\.lat,pal\\.long", predictor) ~ "Paleolatitude &\nPaleolongitude",
    grepl("pal\\.long\\.var", predictor)     ~ "Paleolongitudinal\nspread",
    grepl("pal\\.absLat\\.var", predictor)   ~ "Paleolatitudinal\nspread",
...
p <- ggplot(sampling_heterogeneity_GAM_rmAbundance.deviancePart, aes(x = "", y = deviance, 
                                      fill = label)) +
  geom_bar(stat = "identity", width = 0.7, color = "white") +
  coord_polar("y", clip = "off") +
  ggrepel::geom_label_repel(aes(y = pos, label = label),
...
ggsave("Images/Annex_Deviance_partitionning_rmAbundance.pdf", p, width = 6, height = 6)
p
```
