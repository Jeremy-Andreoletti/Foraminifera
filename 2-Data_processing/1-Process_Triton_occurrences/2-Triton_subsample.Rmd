...
# Occurrences and lineages through time (raw data)
...
triton$bin <- round(triton$age_runif)

# Run SQS subsampling
sqs09_out <- subsample(triton, tax = "species", bin = "bin", type = "sqs", q = 0.9, output = "dist", coll="holeID", ref="holeID", 
                       singleton="ref", excludeDominant = TRUE, largestColl = TRUE, na.rm = T, iter = 100)
sqs06_out <- subsample(triton, tax = "species", bin = "bin", type = "sqs", q = 0.6, output = "dist", coll="holeID", ref="holeID", 
                       singleton="ref", excludeDominant = TRUE, largestColl = TRUE, na.rm = T, iter = 100)
sqs03_out <- subsample(triton, tax = "species", bin = "bin", type = "sqs", q = 0.3, output = "dist", coll="holeID", ref="holeID", 
                       singleton="ref", excludeDominant = TRUE, largestColl = TRUE, na.rm = T, iter = 100)
```

```{r fig.asp=0.6, fig.width=10}
tsplot(stages, shading="short", boxes=c("short", "sys"), boxes.col=c("col", "systemCol"), xlim=c(66,0), ylim=c(0,120), ylab="Range-through diversity", labels.args=list(cex=0.7))
shades(sqs09_out$bin[,1], sqs09_out$divRT, col="darkgreen")
shades(sqs06_out$bin[,1], sqs06_out$divRT, col="palegreen4")
shades(sqs03_out$bin[,1], sqs03_out$divRT, col="palegreen3")
lines(time_points, expertLTT, col="orchid4", lwd=2)

legend("topleft", bg="white", legend=c("SQS (0.9)", "SQS (0.6)", "SQS (0.3)"), col=c("darkgreen", "palegreen4", "palegreen3"), lty=c(1,1,1), lwd=c(2,2,2))

invisible(dev.print(pdf, "Images/Annex_SQS_diversity.pdf"))
```

These SQS diversity curves don't seem to be a great match to the reference LTT.
...
invisible(dev.print(pdf, "Images/OTT.pdf", width=8))
```
