...
list.files("outputs", pattern = "trees", full.names = TRUE)
...
df_files$ltt_path <- sub("_averageRatesThroughTime.csv", "_ltt.csv", df_files$rates_path)
df_files
```
...
for(i in 1:nrow(df_files)){
  ...
  # ----- Read diversity (lineages‐through‐time) --------------------------
  ltt_df    <- read.csv(df_files$ltt_path[i])
  div_cols  <- grep("^ltt_", names(ltt_df), value = TRUE)
  
  ltt_long  <- ltt_df %>%                    
    pivot_longer(all_of(div_cols),
                 names_to  = "replicate",
                 values_to = "diversity") %>%
    mutate(replicate = str_remove(replicate, "ltt_"))
  
  # We'll log‑transform diversity, so avoid zeros
  ltt_long  <- ltt_long %>% 
    mutate(logDiversity = log(diversity),
           original_time = time,
           time = original_time - 1) # add a lag of 1 My
  ...
  birth_data <- rates_df %>% 
    ...
    mutate(replicate = str_remove(replicate, "b_"),
           rate_type = "birth",
           logRate = log(rate))
  ...
  death_data <- rates_df %>% 
    ...
    mutate(replicate = str_remove(replicate, "d_"),
           rate_type = "death",
           logRate = log(rate))
  ...
  # --- Fuzzy join with LTT data ---
  long_data <- long_data %>%
    left_join(ltt_long, by = c("time", "replicate"))
  ...
  for (rt in c("birth", "death")) {
    ...
    fit <- gamm(logRate ~ s(time, replicate, bs="fs") + Temp + SeaLevel + logDiversity,
...
print(correlations)
save(correlations, file = "GAM_Results_Drivers.Rdata")
```

## 4. Plot and Summarize Results

```{r, fig.width=12, fig.asp=0.7, message=FALSE}
...
p_corr <- ggplot(correlations, aes(x = rate_type, y = correlation, fill = rate_type)) +
  geom_boxplot(alpha = 0.8, width = 0.3, size=0.2) +
  ...
  guides(colour = guide_legend(override.aes = list(alpha = 1, lwd = 0.1)))

# Save and display the plot
ggsave("Images/Temporal_correlations.pdf", p_corr, width = 8, height = 5)
print(p_corr + ggtitle("Distribution of Rate-Drivers Correlations with 95% Confidence Intervals"))
```
...
print(cor_summary)
write.csv(cor_summary, file="GAM_Summary_Drivers.csv", row.names = FALSE)
```
